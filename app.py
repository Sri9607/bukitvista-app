...
st.set_page_config(page_title="Prediksi Harga Vila Bukit Vista Menggunakan Model Machine Learning XGBoost", layout="wide")
...
with col2:
    fig, ax = plt.subplots()
    # Dummy contoh: jika ada kolom 'jumlah_malam' gunakan data aktual
    if 'jumlah_malam' in data.columns:
        malam_counts = data['jumlah_malam'].value_counts().sort_index()
        ax.pie(malam_counts.values, labels=[str(x) for x in malam_counts.index], autopct='%1.1f%%', startangle=90)
    else:
        labels = ['1.0', '2.0']
        sizes = [81.2, 18.8]
